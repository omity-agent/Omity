...
test("HookRuntime enforces Drizzle-backed limits and resolves arguments", async () => {
  ...
  const outputs = [{ output: "previous" }];
  const [rule] = hooks.matching("agent", "after");
  if (!rule) {
  ...
  const first = await hooks.execute(rule, "message", "thread", options);
  const second = await hooks.execute(rule, "message", "thread", options);
